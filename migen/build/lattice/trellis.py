...
class LatticeTrellisToolchain:
    ...
    def __init__(self):
        ...
        self.build_template = [
            ...
            "nextpnr-ecp5 --json {build_name}.json --lpf {build_name}.lpf --textcfg {build_name}.config --basecfg {basecfg} --{architecture} --pre-pack {pre_pack_file}",
    ...
    def build(self, platform, fragment, build_dir="build", build_name="top",
        ...
        # generate a pre-pack file that specifies clock domains
        pre_pack_file = build_name + ".py"
        pre_pack_file_contents = "\n".join([
            'ctx.addClock("{}", {})'.format(k, v)
            for k, v in self.freq_constraints.items()
        ])
        tools.write_to_file(pre_pack_file, pre_pack_file_contents)
        ...
        script = _build_script(False, self.build_template, build_name,
                               architecture, basecfg, pre_pack_file)
